...
export async function POST(
  ...
  try {
    ...
    const { data: projectPhotos, error: fetchError } = await supabaseAdmin
      .from('project_photos')
      .select('id')
    ...
    if (fetchError) {
      console.error('[projects/:id/photos/reorder][POST] Failed to load photos:', fetchError);
      return errorResponse('Failed to validate photo order', fetchError.message);
    }

    if ((projectPhotos || []).length !== order.length) {
      return badRequestResponse('One or more photos do not belong to this project');
    }
    ...
    if (updateError) {
      console.error('[projects/:id/photos/reorder][POST] Failed to update order:', updateError);
      return errorResponse('Failed to update photo order', updateError.message);
    }

    return successResponse({ reordered: true });
  } catch (error) {
    console.error('[projects/:id/photos/reorder][POST] Handler error:', error);

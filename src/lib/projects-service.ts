...
const buildSelect = (options: IncludeOptions) => {
  ...
  if (options.units) {
    selections.push(options.photos ? 'units(*,unit_photos(*))' : 'units(*)');
  }
...
export async function fetchProjectByIdentifier(
  identifier: string,
  options: IncludeOptions,
  filters: { isPublic?: boolean | null } = {},
) {
  const selectColumns = buildSelect(options);
  const query = supabaseAdmin.from(PROJECTS_TABLE).select(selectColumns).limit(1);

  const isId = isUuid(identifier);
  let baseQuery = isId ? query.eq('id', identifier) : query.eq('slug', identifier);
  if (typeof filters.isPublic === 'boolean') {
    baseQuery = baseQuery.eq('is_public', filters.isPublic);
  }
...
export async function insertUnitPhoto(
  unitId: string,
  payload: {
    storage_path: string;
    role?: string | null;
